...
var fetchLoadEverything = function(deferred){
    ...
    Q.all(srtPromises).then(function(){
        db
            .put({_id:getCourse(), data:srts})
            .then(function(response){console.log("Successfully stored stuff %o",response); })
            .catch(function(error){
                console.error("Could NOT stored stuff %o",error);
            });
...
var showGUI = function() {
    ...
    $('body').on('click', 'a.fullTextSearch', function(e) {
		...
		function(sectionId, videoLink, time){
		    ...
		    sourceCache[sectionId].src = videoLink+"#t="+time+","+(parseInt(time,10)+10);
		    videoCache[sectionId].load();
		    videoCache[sectionId].play();
		    videoCache[sectionId].scrollIntoViewIfNeeded();
		}(attr["data-section"].value, attr["data-video"].value, attr["data-time"].value);
    ...
    $form.submit(function(e){
        ...
        var sections = _.chain(r)
            .map(function getProperties(o) {
                var t = o.ref.split("#");
                var time = parseInt(t[1], 10), video = parseInt(t[0], 10); 
                var section = videoUrls[video].section;               
                return {time: parseInt(time,10), video:video, section:parseInt(section,10), ref:o.ref}
            })
            .groupBy(function(o){return o.section})
            .sortBy(function(v,k){return parseInt(k,10);})
            .map(function(v,k){return {videos:v, header: videoUrls[v[0].video].header};})
            .value();
            
            
        _.each(sections, function(v, k, o){
            var x = _.chain(v.videos)
                .groupBy(function(o){return o.video;})
                .sortBy("video")
                .map(function(v,k){
                    var t = videoUrls[v[0].video];
                    var quotes = _.chain(v)
                      .sortBy("time").map(function(v){return[v.time,subtitles[v.ref]];}).value();
                    return {title: t.text, link: t.link, quotes: quotes};})
                .value();
            o[k].videos = x;
        });
        picoModal(Handlebars.templates.result(sections)).afterClose(function (modal) { modal.destroy(); }).show();
        return false;
    });
